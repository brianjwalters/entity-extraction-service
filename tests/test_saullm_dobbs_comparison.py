#!/usr/bin/env python3
"""
SaulLM Dobbs Document Comparison Test
Tests SaulLM-7B entity extraction on the Dobbs v. Jackson Supreme Court case document.
"""
...
from rahimi_processor import DocumentProcessor
...
class SaulLMDobbsExtractor:
    """SaulLM-based entity extractor specifically for Dobbs document testing."""
    ...
    def __init__(self, model_name: str = "Equall/Saul-7B-Instruct-v1"):
        ...
        # Legal entity types for SaulLM (enhanced for reproductive rights cases)
        self.entity_types = {
            'CASE_CITATION': 'Case names with full citations (e.g., Roe v. Wade, 410 U.S. 113)',
            'STATUTE': 'Federal and state statutory references and codes',
            'REGULATION': 'CFR and administrative regulations',
            'CONSTITUTIONAL': 'Constitutional provisions, amendments, and clauses',
            ...
            'LEGAL_DOCTRINE': 'Legal principles, doctrines, and constitutional rights',
            'PROCEDURAL_TERM': 'Motions, orders, procedural actions',
            'JURISDICTION': 'Geographic and legal jurisdictions',
            'LEGAL_STANDARD': 'Standards of review, legal tests, and scrutiny levels',
            'MEDICAL_TERM': 'Medical procedures and terminology in legal context',
            'STATE_LAW': 'State-specific laws and regulations'
        }
    ...
    def create_dobbs_extraction_prompt(self, text: str) -> str:
        """Create specialized prompt for legal entity extraction from Dobbs document."""
        
        entity_descriptions = "\n".join([
            f"- {etype}: {desc}" for etype, desc in self.entity_types.items()
        ])
        
        prompt = f"""You are an expert legal entity extraction system specializing in Supreme Court cases, particularly those involving reproductive rights and constitutional law. Extract ALL legal entities from the following Dobbs v. Jackson Supreme Court document text with maximum precision and recall.
...
4. Capture reproductive rights terminology and medical terms in legal context
5. Identify state laws and regulations mentioned
6. Return entities as a JSON array only

## Supreme Court Document Text (Dobbs v. Jackson):
"{text}"
...
Entities:"""
    ...
    def extract_entities_from_text(self, text: str) -> List[Dict]:
        ...
        # Create prompt
        prompt = self.create_dobbs_extraction_prompt(text)
    ...
    def _fallback_entity_parsing(self, text: str) -> List[Dict]:
        """Fallback parsing when JSON fails - enhanced for Dobbs case."""
        import re
        entities = []
        
        # Look for common Supreme Court and reproductive rights patterns
        patterns = {
            'CASE_CITATION': [
                r'[A-Z][a-z]+ v\. [A-Z][a-z]+,?\s+\d+\s+U\.?\s*S\.?\s+\d+',
                r'Roe v\. Wade',
                r'Planned Parenthood v\. Casey',
                r'Dobbs v\. Jackson',
            ],
            'JUDGE': [
                r'Justice [A-Z][a-z]+',
            ...
            'CONSTITUTIONAL': [
                r'(First|Second|Fourth|Fifth|Sixth|Eighth|Ninth|Tenth|Fourteenth) Amendment',
                r'Due Process Clause',
                r'Equal Protection Clause',
                r'substantive due process',
            ],
            'STATUTE': [
                r'\d+\s+U\.S\.C\.?\s+§\s*\d+',
                r'Section \d+',
            ...
            'STATE_LAW': [
                r'Mississippi\s+(?:law|statute|code)',
                r'Texas\s+(?:law|statute|code)',
            ],
            'MEDICAL_TERM': [
                r'abortion',
                r'fetal viability',
    ...
    def process_dobbs_document(self, chunk_size: int = 4000) -> Dict[str, Any]:
        """Process the complete Dobbs document."""
        logger.info("Starting SaulLM Dobbs document processing...")
        
        # Get Dobbs content
        processor = DocumentProcessor("dobbs")
        ...
        results_file = self.results_dir / f"saullm_dobbs_results_{timestamp}.json"
...
def main():
    """Main function to run SaulLM Dobbs comparison test."""
    print("="*80)
    print("SAULLM DOBBS DOCUMENT EXTRACTION TEST")
    print("="*80)
    
    try:
        # Initialize extractor
        extractor = SaulLMDobbsExtractor()
        
        # Process Dobbs document
        results = extractor.process_dobbs_document()
